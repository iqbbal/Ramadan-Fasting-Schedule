...
A Windows system tray (taskbar) application that displays
Ramadan fasting schedule based on IP geolocation or manual city selection.

Features:
- System tray icon with crescent moon
- Right-click menu showing all prayer times
- Countdown to next Imsak/Iftar
- IP-based auto location detection
- Manual city selection settings
- Desktop notifications for Imsak & Iftar
- Uses Aladhan API (free, no API key needed)
- Uses Kementerian Agama Indonesia calculation method

Author: Ramadhan Windows Taskbar PoC
"""
import sys
import os
import time
...
LOCK_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".ramadhan_tray.pid")
...
from notification_service import NotificationScheduler
from custom_menu import build_dark_menu
...
class RamadhanTrayApp:
    ...
    def _update_icon(self):
        ...
        try:
            ...
            if not imsak_past:
                # Show countdown to imsak
                new_icon = create_countdown_icon(imsak_h, imsak_m)
            elif not maghrib_past:
                # Show countdown to maghrib (iftar)
    ...
    def _update_loop(self):
        ...
        while self.running:
            try:
                # Refresh data if new day or interval elapsed
                today = date.today()
    ...
    def _on_settings(self, icon=None, item=None):
        ...
        # Run settings in a separate thread to not block tray
